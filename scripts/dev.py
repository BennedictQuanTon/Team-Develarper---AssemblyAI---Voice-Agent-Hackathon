...
def start(args: argparse.Namespace) -> int:
    ...
    if not (ROOT / ".env").exists() and (ROOT / ".env.example").exists():
        ...
        print("Created .env from .env.example; add your API keys (without them every vendor client is a stub).")
    ensure_frontend()
...
def status(args: argparse.Namespace) -> int:
    ...
    if health:
        print(f"  client_mode={health.get('client_mode')}  keys_configured={health.get('keys_configured')}")
    base = setting("OLLAMA_BASE_URL", "http://localhost:11434").rstrip("/")
    tags = http_json(f"{base}/api/tags", timeout=2)
    if tags is None:

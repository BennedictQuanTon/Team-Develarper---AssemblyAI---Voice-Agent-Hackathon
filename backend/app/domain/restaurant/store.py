...
RESTAURANT_DIR = ROOT_DIR / "data" / "restaurant"
# Kept as an import-compatible alias while callers migrate to RESTAURANT_DIR.
LANTERN_DIR = RESTAURANT_DIR
...
class LanternStore:
    """Thread-safe in-memory restaurant state, seeded from data/restaurant/*.json."""

    def __init__(self, restaurant_dir: Path = RESTAURANT_DIR):
        self._dir = restaurant_dir
    ...
    def _load_tables(self) -> dict[str, Table]:
        path = self._dir / "floor_tables.json"
        with path.open(encoding="utf-8") as handle:
            payload = json.load(handle)
    ...
    def _load_schedule(self) -> dict[str, Any]:
        path = self._dir / "service_schedule.json"
        with path.open(encoding="utf-8") as handle:

...
Outputs an immutable restaurant/context-fillers run directory with a manifest,
structured results, and a readable summary.
"""
...
AUDIO_DIR = Path(__file__).resolve().parents[3] / "frontend" / "public" / "audio" / "backchannels"
...
def main():
    run = create_run(product="restaurant", suite="context-fillers")
    print("==================================================================")
    ...
    # Write JSON Report
    json_path = write_results(run, full_report)
...
> **Dữ liệu JSON chi tiết**: [`results.json`](results.json)
...
"""
    md_path = write_summary(run, md_content)

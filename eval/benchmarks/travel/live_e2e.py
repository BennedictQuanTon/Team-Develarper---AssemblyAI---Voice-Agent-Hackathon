...
Usage (server optional — calls Orchestrator in-process):
  PYTHONPATH=. .venv/bin/python eval/benchmarks/travel/live_e2e.py
...
from legacy.travel.rag.retrieve import hybrid_retrieve

QRELS_PATH = ROOT_DIR / "eval" / "datasets" / "travel" / "rag_qrels.v1.json"
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=None, help="Optional explicit results JSON path")
    parser.add_argument("--audio-dir", type=Path, default=ROOT_DIR / "var" / "benchmark-audio")
    ...
    from legacy.travel.rag.retrieve import warmup_retriever
    ...
    if args.out is not None:
        ...
        run = create_run(product="legacy-travel", suite="live-e2e", run_id=args.run_id)
        out_path = write_results(run, report)

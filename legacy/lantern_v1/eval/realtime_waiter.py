...
Run:
  PYTHONPATH=. .venv/bin/python eval/smoke/restaurant/order_flows.py  # Part A only
  # start server first, then:
  PYTHONPATH=. .venv/bin/python eval/benchmarks/restaurant/realtime_waiter.py --realtime
  PYTHONPATH=. .venv/bin/python eval/benchmarks/restaurant/realtime_waiter.py --all

Writes an immutable restaurant/realtime-waiter run directory.
"""
...
from eval.reporting import BenchmarkRun, create_run, write_results, write_summary

_run: BenchmarkRun | None = None


def _report_run() -> BenchmarkRun:
    global _run
    if _run is None:
        _run = create_run(product="restaurant", suite="realtime-waiter")
    return _run
...
async def run_full_flow(settings) -> dict:
    ...
    out_json = _report_run().directory / "latency-turns.json"
    out_json.write_text(json.dumps(detail, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
...
async def run_one_scenario(settings) -> dict:
    ...
    out_json = _report_run().results_path
    write_results(_report_run(), detail)
    md = [
    ...
    out_md = _report_run().directory / "native-flow-summary.md"
    out_md.write_text("\n".join(md) + "\n", encoding="utf-8")
    print(f"\nWrote detail JSON -> {out_json}")
...
def write_report(part_a: dict, part_b: dict):
    ...
    report_path = write_summary(_report_run(), "\n".join(lines))

...
> **Raw data**: [`reports/benchmark_today_eval.json`](benchmark_today_eval.json) ·
> [`reports/benchmark_today_report.md`](benchmark_today_report.md)
>
...
`uv run python -m eval.benchmark_today` — 6-turn ordering conversation, real AssemblyAI + Gemini + Cartesia,
compared against the recorded baseline in `reports/waiter_e2e_detail.json`:
...
uv run python -m eval.benchmark_today                  # 6 turns vs baseline
